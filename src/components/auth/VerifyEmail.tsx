...
export default function VerifyEmail({ token }: Props) {
  ...
  useEffect(() => { handleVerifyEmail(); }, [handleVerifyEmail]);
  ...
  return (
    <Card className="max-w-md w-full text-center">
      <CardHeader className="flex flex-col items-center">
        {isError ? <XCircle className="w-10 h-10 text-red-500" /> : <CheckCircle className="w-10 h-10 text-green-500" />}
      ...
      <CardFooter className="justify-center">
        <Button asChild size="lg"><Link href="/">Accéder au jeu</Link></Button>
      </CardFooter>
    </Card>
  );
}
